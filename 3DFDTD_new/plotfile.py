...
def setup(FLAG, fig, ax, ddx, dt, length, array, dims, sphere, pulse, npml, tfsf,e, ez_inc, hx_inc):
    ...
    plot_text += '\n\n Pulse information\n Amplitude: ' + str(pulse.amplitude) \
        ...
        + '\n dt: {}'.format(np.round(dt*1e18,2)) + ' as'
    ...
    xcut, = ax[0,2].plot(array.x/nm,np.abs(e.z[:,int(dims.y/2),int(dims.z/2)]), label='X cut')
    ycut, = ax[1,2].plot(array.x/nm,np.abs(e.z[int(dims.x/2),:,int(dims.z/2)]), label='Y cut')
    zcut, = ax[2,2].plot(array.x/nm,np.abs(e.z[int(dims.x/2),int(dims.y/2),:]), label='Z cut')
    ...
    fx = plt.Circle((sphere.x/nm, sphere.y/nm), sphere.R/nm, color='r',fill=False)
    fy = plt.Circle((sphere.x/nm, sphere.y/nm), sphere.R/nm, color='r',fill=False)
    fz = plt.Circle((sphere.x/nm, sphere.y/nm), sphere.R/nm, color='r',fill=False)
    px = plt.Circle((sphere.x/nm, sphere.y/nm), sphere.R/nm, color='r',fill=False)
    py = plt.Circle((sphere.x/nm, sphere.y/nm), sphere.R/nm, color='r',fill=False)
    pz = plt.Circle((sphere.x/nm, sphere.y/nm), sphere.R/nm, color='r',fill=False)
    ...
    ax[2,1].hlines(npml*ddx/nm,npml*ddx/nm,(dims.x-npml-1)*ddx/nm, 'b')
    ax[2,1].hlines((dims.x-npml-1)*ddx/nm,npml*ddx/nm,(dims.x-npml-1)*ddx/nm, 'b')
    ax[2,1].vlines(npml*ddx/nm,npml*ddx/nm,(dims.x-npml-1)*ddx/nm, 'b')
    ax[2,1].vlines((dims.x-npml-1)*ddx/nm,npml*ddx/nm,(dims.x-npml-1)*ddx/nm, 'b')
    
    #TFSF
    ax[0,1].hlines(tfsf.x_min*ddx/nm,tfsf.z_min*ddx/nm,(tfsf.z_max+1)*ddx/nm, 'r')
    ax[0,1].hlines((tfsf.x_max+1)*ddx/nm,tfsf.z_min*ddx/nm,(tfsf.z_max+1)*ddx/nm, 'r')
    ax[0,1].vlines(tfsf.z_min*ddx/nm,tfsf.x_min*ddx/nm,(tfsf.x_max+1)*ddx/nm, 'r')
    ax[0,1].vlines((tfsf.z_max+1)*ddx/nm,tfsf.x_min*ddx/nm,(tfsf.x_max+1)*ddx/nm, 'r')
    ...
    ax[2,1].hlines(tfsf.x_min*ddx/nm,tfsf.z_min*ddx/nm,(tfsf.z_max+1)*ddx/nm, 'r')
    ax[2,1].hlines((tfsf.x_max+1)*ddx/nm,tfsf.z_min*ddx/nm,(tfsf.z_max+1)*ddx/nm, 'r')
    ax[2,1].vlines(tfsf.z_min*ddx/nm,tfsf.x_min*ddx/nm,(tfsf.x_max+1)*ddx/nm, 'r')
    ax[2,1].vlines((tfsf.z_max+1)*ddx/nm,tfsf.x_min*ddx/nm,(tfsf.x_max+1)*ddx/nm, 'r')

    return ims, imp, text_tstep, xcut, ycut, zcut, incident_e, incident_h


def animate(t,text_tstep,e,dims,ims,array,ax,xcut,ycut,zcut,incident_e,ez_inc,hx_inc,incident_h,p,imp,time_pause):
...
def setup_GIF(FLAG, fig, ax, ddx, dt, length, array, dims, sphere, pulse, npml, tfsf,e, ez_inc, hx_inc,f_plot):
    ...
    ims.append(ax[1,1].imshow(np.zeros((dims.x, 1)), cmap='viridis',
                                interpolation='quadric', origin='lower'))
    ims.append(ax[1,2].imshow(np.zeros((1, 1)), cmap='viridis',
                                interpolation='quadric', origin='lower'))
    ...
    # Jonas plot 
    plot_f, = ax[0,1].plot(grid.k_grid(grid.n_kmax),f_plot[:,0,int(grid.n_thetamax/2)],label='f_k(r=0)')
    plot_f2, = ax[0,1].plot(grid.k_grid(grid.n_kmax),f_plot[:,int(grid.n_phimax/4),int(grid.n_thetamax/2)],label='f_k(r=0)')
    plot_f3, = ax[0,1].plot(grid.k_grid(grid.n_kmax),f_plot[:,0,0],label='f_k(r=0)')
    ...
    ax[0,2].plot(time_array*dt*1e15,pulse.update_value(time_array,dt),label='Pulse')
    incident_loc, = ax[0,2].plot(time_array[10]*dt, pulse.update_value(time_array[10],dt), 'bo')
    ...
    imp.append(ax[2,1].imshow(np.zeros((dims.x, 1)), cmap='viridis',
                                interpolation='quadric', origin='lower'))

    imp.append(ax[2,2].imshow(np.zeros((1, 1)), cmap='viridis',
                                interpolation='quadric', origin='lower'))
...
def animate_GIF(t,text_tstep,e,dims,ims,array,ax,ez_inc,hx_inc,incident_loc,pulse,p,imp,time_pause,f_plot,plot_f,plot_f2,plot_f3):
    text_tstep.set_text('Time Step: ' + str(t))
    max = np.max(np.abs(e.z[:,:,int(dims.y/2)]))
    #print('max',max)
    for im in ims:
    ...
    plot_f2.set_data(grid.k_grid(grid.n_kmax),f_plot[:,int(grid.n_phimax/4),int(grid.n_thetamax/2)])
