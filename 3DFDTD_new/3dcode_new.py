...
# create pulse
pulse = Pulse(width=2,delay = 1.7,energy=2.3,dt = dt,ddx = ddx, eps_in = eps_in)             # Optical pulse
...
if FLAG.MICRO ==1:
    ...
    f_global[:,:,:] = np.reshape(initial_conditions.load_initial_conditions(FLAG.INIT, grid.E_F, grid.T_ele), (grid.n_kmax, grid.n_phimax, grid.n_thetamax))
    ...
    in_scattering_matrix_abs, in_scattering_matrix_em, out_scattering_matrix_abs, out_scattering_matrix_em, kp_matrix = boltzmann.calculate_scattering_matrices(in_scattering_matrix_abs, in_scattering_matrix_em, out_scattering_matrix_abs, out_scattering_matrix_em, kp_matrix, FLAG.ELPHO)
    ...
    grad_matrix_k, grad_matrix_phi, grad_matrix_theta, unitvector_k, unitvector_phi, unitvector_theta, funcdet_phi, funcdet_theta = boltzmann.calculate_light_matter_matrices(grad_matrix_k, grad_matrix_phi, grad_matrix_theta, unitvector_k, unitvector_phi, unitvector_theta, funcdet_phi, funcdet_theta, FLAG.LM, FLAG.STENCIL)
...
if FLAG.OBJECT == 1: # if circle
    ...
    if FLAG.MATERIAL == 2: # DrudeLorentz
        ...
        ga, l1, l2, l3 = object.create_sphere_lorentz(sphere,nsub,ddx,dt,eps_in,eps_out,wl,gamma_l,delta_eps,ga,l1,l2,l3)
    
    if FLAG.MATERIAL == 3: # Etchegoin 
        ga, d1, d2, d3 = object.create_sphere_drude_eps(
            sphere, nsub, ddx, dt, eps_in, eps_out, wp, gamma, ga, d1, d2, d3
        )
        f1_et1, f2_et1, f3_et1, f4_et1, f1_et2, f2_et2, f3_et2,f4_et2 = object.create_sphere_etch(sphere,nsub,ddx,dt,c1,c2,w1,w2,gamma1,gamma2,f1_et1,f2_et1,f3_et1,f4_et1,f1_et2,f2_et2,f3_et2,f4_et2)

elif FLAG.OBJECT ==2 and FLAG.TFSF ==2:

    if FLAG.MATERIAL == 1: # Drude model
        ga, d1, d2, d3 = object.create_rectangle_PBC(dims,int(dims.y/2),int(dims.y/2+sphere.R/ddx),dt,eps_in,wp,gamma,ga,d1,d2,d3) # should derive a different quantity for the diameter instead of using sphere radius
    
    if FLAG.MATERIAL == 2: # DrudeLorentz
        ga, d1, d2, d3 = object.create_rectangle_PBC(dims,int(dims.y/2),int(dims.y/2+sphere.R/ddx),dt,eps_in,wp,gamma,ga,d1,d2,d3)
        ga, l1, l2, l3 = object.create_rectangle_PBC_lorentz(dims,int(dims.y/2),int(dims.y/2+sphere.R/ddx),dt,eps_in,wl,gamma_l,delta_eps,ga,l1,l2,l3)
    
    if FLAG.MATERIAL == 3: # Etchegoin
        ga, d1, d2, d3 = object.create_rectangle_PBC(dims,int(dims.y/2),int(dims.y/2+sphere.R/ddx),dt,eps_in,wp,gamma,ga,d1,d2,d3)
        f1_et1, f2_et1, f3_et1, f4_et1, f1_et2, f2_et2, f3_et2, f4_et2 = object.create_rectangle_PBC_etch(dims,int(dims.y/2),int(dims.y/2+sphere.R/ddx),dt,c1,c2,w1,gamma1,w2,gamma2,f1_et1,f2_et1,f3_et1,f4_et1,f1_et2,f2_et2,f3_et2,f4_et2)
...
ims, imp, text_tstep, xcut, ycut, zcut, incident_e, incident_h = plots.setup(FLAG, fig, ax, ddx, dt, length, array, dims, sphere, pulse, npml, tfsf,e, ez_inc, hx_inc)
...
for time_step in range(1,tsteps+1):
    ...
    if FLAG.MICRO == 1 and (time_step-1) % time_micro_factor == 0:
        ...
        j, f_global = object.update_micro(ddx,time_micro_factor*dt,offset,diameter,j,f_global,e,sphere, FLAG.E_FIELD, ep_in_a, ep_in_e, ep_out_a, ep_out_e, kp, lm_k, lm_p, lm_t, e_k, e_p, e_t, fd_p, fd_t)  
    ...
    if(FLAG.OBJECT!=0 and FLAG.MICRO ==0):
        if FLAG.MATERIAL == 1: # Drude only
            p_drude, p_tmp_drude = object.calculate_polarization(dims,sphere,ddx,p_drude,p_tmp_drude,e,d1,d2,d3,FLAG.OBJECT)
            p.x = p_drude.x
            p.y = p_drude.y 
            p.z = p_drude.z
        ...
        if FLAG.MATERIAL == 2: # DrudeLorentz: 
            p_drude, p_tmp_drude = object.calculate_polarization(dims,sphere,ddx,p_drude,p_tmp_drude,e,d1,d2,d3,FLAG.OBJECT)
            p_lorentz, p_tmp_lorentz = object.calculate_polarization(dims,sphere,ddx,p_lorentz,p_tmp_lorentz,e,l1,l2,l3,FLAG.OBJECT)
        ...
        if FLAG.MATERIAL == 3:  # Etchegoin 
            p_drude, p_tmp_drude = object.calculate_polarization(dims,sphere,ddx,p_drude,p_tmp_drude,e,d1,d2,d3,FLAG.OBJECT)
            p_et1, p_tmp_et1 = object.calculate_polarization_etch(dims,sphere,ddx,p_et1,p_tmp_et1,e,e1,f1_et1,f2_et1,f3_et1,f4_et1,FLAG.OBJECT)
            p_et2, p_tmp_et2 = object.calculate_polarization_etch(dims,sphere,ddx,p_et2,p_tmp_et2,e,e1,f1_et2,f2_et2,f3_et2,f4_et2,FLAG.OBJECT)
    ...
    if(FLAG.POINT==1):
        ...
        pulsemon_t, ez_source_t = mnt.update_pulsemonitors(
            time_step, e.z, source.x, source.y, source.z, pulse_tmp, pulsemon_t, ez_source_t
        )
    'Update 1d buffer for plane wave '
    if(FLAG.TFSF==1 or FLAG.TFSF ==2):
    ...
    if FLAG.DFT2D == 1 and time_step > dft.tstart:
        ...
        HxReDFT_znormal,HxImDFT_znormal,HyReDFT_znormal,HyImDFT_znormal,HzReDFT_znormal,HzImDFT_znormal = mnt.DFT2D_update(e,h,dft.iwmax,dft.omega,time_step,x_DFT,y_DFT,z_DFT,\
                ExReDFT_xnormal,ExImDFT_xnormal,EyReDFT_xnormal,EyImDFT_xnormal,EzReDFT_xnormal,EzImDFT_xnormal,\
                HxReDFT_xnormal,HxImDFT_xnormal,HyReDFT_xnormal,HyImDFT_xnormal,HzReDFT_xnormal,HzImDFT_xnormal,\
                ExReDFT_ynormal,ExImDFT_ynormal,EyReDFT_ynormal,EyImDFT_ynormal,EzReDFT_ynormal,EzImDFT_ynormal,\
                HxReDFT_ynormal,HxImDFT_ynormal,HyReDFT_ynormal,HyImDFT_ynormal,HzReDFT_ynormal,HzImDFT_ynormal,\
    ...
    if(FLAG.TFSF ==2 and time_step > dft.tstart):
        e_ref, e_trans = mnt.DFT_ref_trans(e_ref, e_trans,e,y_ref,y_trans,dft.iwmax,dft.omega,time_step)
    ...
    if(FLAG.CROSS == 1 and time_step > dft.tstart):
        ...
        e_scat_z_min,e_scat_z_max,h_scat_z_min,h_scat_z_max = mnt.DFT_scat_update(
                                ...
                                e,h,scat,dft.iwmax,dft.omega,time_step)
    ...
    if(FLAG.CROSS == 1 and time_step > dft.tstart):
        ...
        e_abs_z_min,e_abs_z_max,h_abs_z_min,h_abs_z_max = mnt.DFT_abs_update(
                                ...
                                e,h,abs,dft.iwmax,dft.omega,time_step)
...
if(FLAG.CROSS == 1): 
    S_scat_DFT = mnt.update_scat_DFT(S_scat_DFT,dft.iwmax,scat,\
                    e_scat_x_min,e_scat_x_max,h_scat_x_min,h_scat_x_max,\
                    e_scat_y_min,e_scat_y_max,h_scat_y_min,h_scat_y_max,\
                    e_scat_z_min,e_scat_z_max,h_scat_z_min,h_scat_z_max)

    S_abs_DFT = mnt.update_abs_DFT(S_abs_DFT,dft.iwmax,abs,
                    e_abs_x_min,e_abs_x_max,h_abs_x_min,h_abs_x_max,\
                    e_abs_y_min,e_abs_y_max,h_abs_y_min,h_abs_y_max,\
                    e_abs_z_min,e_abs_z_max,h_abs_z_min,h_abs_z_max)
...
if FLAG.POINT ==1:
    data.store_point(FLAG,sphere,ddx,dt,tsteps,dims,pulse,dft,npml,eps_in,eps_out,source,pulsemon_t,ez_source_t,start,stop)
if FLAG.CROSS ==1:
    data.store_cross(FLAG,sphere,ddx,dt,tsteps,dims,pulse,dft,npml,eps_in,eps_out,start,stop,
                S_scat_total, S_abs_total, SourceReDFT, SourceImDFT, wp, gamma, tfsf_dist)

if FLAG.TFSF ==2 and FLAG.MICRO ==0:
    data.store_periodic(FLAG,sphere,ddx,dt,tsteps,dims,pulse,dft,npml,eps_in,eps_out,start,stop,
                SourceReDFT, SourceImDFT, wp, gamma, tfsf_dist, transmission, reflection)
if FLAG.TFSF ==2 and FLAG.MICRO ==1:
    data.store_periodic_micro(FLAG,sphere,ddx,dt,tsteps,dims,pulse,dft,npml,eps_in,eps_out,start,stop,
                SourceReDFT, SourceImDFT, tfsf_dist, transmission, reflection)