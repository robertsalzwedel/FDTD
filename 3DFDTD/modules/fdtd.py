    ...
    for i in range(0, dims.x):
        for j in range(1, dims.y):
            for k in range(1, dims.z):
                curlH = (
                    h.z[i, j, k] - h.z[i, j - 1, k] - h.y[i, j, k] + h.y[i, j, k - 1]
                )
                idx[i, j, k] = curlH + idx[i, j, k]
                dx[i, j, k] = pml.gj3[j] * pml.gk3[k] * dx[i, j, k] + pml.gj2[
                    j
                ] * pml.gk2[k] * (0.5 * curlH + pml.gi1[i] * idx[i, j, k])
    return dx, idx
...
def calculate_dy_field(dims, dy, h, idy, pml):
    ...
    for i in range(1, dims.x):
        for j in range(0, dims.y):
            for k in range(1, dims.z):
                curlH = (
                    h.x[i, j, k] - h.x[i, j, k - 1] - h.z[i, j, k] + h.z[i - 1, j, k]
                )
                idy[i, j, k] = curlH + idy[i, j, k]
                dy[i, j, k] = pml.gi3[i] * pml.gk3[k] * dy[i, j, k] + pml.gi2[
                    i
                ] * pml.gk2[k] * (0.5 * curlH + pml.gj1[j] * idy[i, j, k])
    return dy, idy
    ...
    for i in range(1, dims.x):
        for j in range(1, dims.y):
            for k in range(0, dims.z):
                curlH = (
                    h.y[i, j, k] - h.y[i - 1, j, k] - h.x[i, j, k] + h.x[i, j - 1, k]
                )
                idz[i, j, k] = curlH + idz[i, j, k]
                dz[i, j, k] = pml.gi3[i] * pml.gj3[j] * dz[i, j, k] + pml.gi2[
                    i
                ] * pml.gj2[j] * (0.5 * curlH + pml.gk1[k] * idz[i, j, k])
    return dz, idz
...
def calculate_dx_field_PBC(dims, dx, h, idx, pml):
    ...
    for i in range(0, dims.x):
        for j in range(1, dims.y):
            curlH = h.z[i, j, 0] - h.z[i, j - 1, 0] - h.y[i, j, 0] + h.y[i, j, -1]
            idx[i, j, 0] = curlH + idx[i, j, 0]
            dx[i, j, 0] = pml.gj3[j] * pml.gk3[0] * dx[i, j, 0] + pml.gj2[j] * pml.gk2[
                0
            ] * (0.5 * curlH + pml.gi1[i] * idx[i, j, 0])
    return dx, idx
...
def calculate_dy_field_PBC(dims, dy, h, idy, pml):
    ...
    for j in range(0, dims.y):
        for i in range(1, dims.x):
            curlH = h.x[i, j, 0] - h.x[i, j, -1] - h.z[i, j, 0] + h.z[i - 1, j, 0]
            idy[i, j, 0] = curlH + idy[i, j, 0]
            dy[i, j, 0] = pml.gi3[i] * pml.gk3[0] * dy[i, j, 0] + pml.gi2[i] * pml.gk2[
                0
            ] * (0.5 * curlH + pml.gj1[j] * idy[i, j, 0])
        for k in range(1, dims.z):
            curlH = h.x[0, j, k] - h.x[0, j, k - 1] - h.z[0, j, k] + h.z[-1, j, k]
            idy[0, j, k] = curlH + idy[0, j, k]
            dy[0, j, k] = pml.gi3[0] * pml.gk3[k] * dy[0, j, k] + pml.gi2[0] * pml.gk2[
                k
            ] * (0.5 * curlH + pml.gj1[j] * idy[0, j, k])
    return dy, idy
...
def calculate_dz_field_PBC(dims, dz, h, idz, pml):
    ...
    for j in range(1, dims.y):
        for k in range(0, dims.z):
            curlH = h.y[0, j, k] - h.y[-1, j, k] - h.x[0, j, k] + h.x[0, j - 1, k]
            idz[0, j, k] = curlH + idz[0, j, k]
            dz[0, j, k] = pml.gi3[0] * pml.gj3[j] * dz[0, j, k] + pml.gi2[0] * pml.gj2[
                j
            ] * (0.5 * curlH + pml.gk1[k] * idz[0, j, k])
    return dz, idz
    ...
    for i in range(0, dims.x):
        for j in range(0, dims.y - 1):
            for k in range(0, dims.z - 1):
                curlE = (
                    e.y[i, j, k + 1] - e.y[i, j, k] - e.z[i, j + 1, k] + e.z[i, j, k]
                )
                ihx[i, j, k] = curlE + ihx[i, j, k]
                hx[i, j, k] = pml.fj3[j] * pml.fk3[k] * hx[i, j, k] + pml.fj2[
                    j
                ] * pml.fk2[k] * (0.5 * curlE + pml.fi1[i] * ihx[i, j, k])
    return hx, ihx
...
def calculate_hy_field(dims, hy, e, ihy, pml):
    for i in range(0, dims.x - 1):
        for j in range(0, dims.y):
            for k in range(0, dims.z - 1):
                curlE = (
                    e.z[i + 1, j, k] - e.z[i, j, k] - e.x[i, j, k + 1] + e.x[i, j, k]
                )
                ihy[i, j, k] = curlE + ihy[i, j, k]
                hy[i, j, k] = pml.fi3[i] * pml.fk3[k] * hy[i, j, k] + pml.fi2[
                    i
                ] * pml.fk2[k] * (0.5 * curlE + pml.fj1[j] * ihy[i, j, k])
    return hy, ihy
...
def calculate_hz_field(dims, hz, e, ihz, pml):
    ...
    for i in range(0, dims.x - 1):
        for j in range(0, dims.y - 1):
            for k in range(0, dims.z):
                curlE = (
                    e.x[i, j + 1, k] - e.x[i, j, k] - e.y[i + 1, j, k] + e.y[i, j, k]
                )
                ihz[i, j, k] = curlE + ihz[i, j, k]
                hz[i, j, k] = pml.fi3[i] * pml.fj3[j] * hz[i, j, k] + pml.fi2[
                    i
                ] * pml.fj2[j] * (0.5 * curlE + pml.fk1[k] * ihz[i, j, k])
    return hz, ihz
...
def calculate_hx_field_PBC(dims, hx, e, ihx, pml):
    ...
    for i in range(0, dims.x):
        for j in range(0, dims.y - 1):
            ...
            hx[i, j, dims.z - 1] = pml.fj3[j] * pml.fk3[dims.z - 1] * hx[
                i, j, dims.z - 1
            ] + pml.fj2[j] * pml.fk2[dims.z - 1] * (
                0.5 * curlE + pml.fi1[i] * ihx[i, j, dims.z - 1]
            )
    return hx, ihx
...
def calculate_hy_field_PBC(dims, hy, e, ihy, pml):
    ...
    for j in range(0, dims.y):
        for i in range(0, dims.x - 1):
            ...
            hy[i, j, dims.z - 1] = pml.fi3[i] * pml.fk3[dims.z - 1] * hy[
                i, j, dims.z - 1
            ] + pml.fi2[i] * pml.fk2[dims.z - 1] * (
                0.5 * curlE + pml.fj1[j] * ihy[i, j, dims.z - 1]
            )
        for k in range(0, dims.z - 1):
            curlE = (
                e.z[0, j, k]
            ...
            hy[dims.x - 1, j, k] = pml.fi3[dims.x - 1] * pml.fk3[k] * hy[
                dims.x - 1, j, k
            ] + pml.fi2[dims.x - 1] * pml.fk2[k] * (
                0.5 * curlE + pml.fj1[j] * ihy[dims.x - 1, j, k]
            )
    return hy, ihy
...
def calculate_hz_field_PBC(dims, hz, e, ihz, pml):
    ...
    for j in range(0, dims.y - 1):
        for k in range(0, dims.z):
            ...
            hz[dims.x - 1, j, k] = pml.fi3[dims.x - 1] * pml.fj3[j] * hz[
                dims.x - 1, j, k
            ] + pml.fi2[dims.x - 1] * pml.fj2[j] * (
                0.5 * curlE + pml.fk1[k] * ihz[dims.x - 1, j, k]
            )
    return hz, ihz
...
def D_update(args, dims, d, h, id, PML, tfsf, hx_inc):
    """Update D Fields"""
...
def H_update(args, dims, h, ih, e, PML, tfsf, ez_inc):
    """Update H fields"""
