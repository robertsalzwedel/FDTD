...
def DFT_incident_update(omegaDFT, SourceDFT, pulse, iwdim, t):
    for om in range(0, iwdim + 1):
        SourceDFT[0, om] = SourceDFT[0, om] + np.cos(omegaDFT[om] * t) * pulse
        SourceDFT[1, om] = SourceDFT[1, om] - np.sin(omegaDFT[om] * t) * pulse
    return SourceDFT
...
def update_1Dmonitors(
    t, loc_monitors, ex_mon, ey_mon, ez_mon, hx_mon, hy_mon, hz_mon, e, h
):
    ...
    for om in range(iwdim + 1):
        ...
        for y in range(scat.y_min, scat.y_max + 1):
            for z in range(scat.z_min, scat.z_max + 1):
                S_scat_DFT[0, om] -= 0.5 * (
                    np.real(
                        e_scat_x_min.y[om, y, z]
                        * np.conjugate(h_scat_x_min.z[om, y, z])
                    )
                    - np.real(
                        e_scat_x_min.z[om, y, z]
                        * np.conjugate(h_scat_x_min.y[om, y, z])
                    )
                )
                S_scat_DFT[1, om] += 0.5 * (
                    np.real(
                        e_scat_x_max.y[om, y, z]
                        * np.conjugate(h_scat_x_max.z[om, y, z])
                    )
                    - np.real(
                        e_scat_x_max.z[om, y, z]
                        * np.conjugate(h_scat_x_max.y[om, y, z])
                    )
                )
        ...
        for x in range(scat.x_min, scat.x_max + 1):
            for z in range(scat.z_min, scat.z_max + 1):
                S_scat_DFT[2, om] -= 0.5 * (
                    np.real(
                        e_scat_y_min.z[om, x, z]
                        * np.conjugate(h_scat_y_min.x[om, x, z])
                    )
                    - np.real(
                        e_scat_y_min.x[om, x, z]
                        * np.conjugate(h_scat_y_min.z[om, x, z])
                    )
                )
                S_scat_DFT[3, om] += 0.5 * (
                    np.real(
                        e_scat_y_max.z[om, x, z]
                        * np.conjugate(h_scat_y_max.x[om, x, z])
                    )
                    - np.real(
                        e_scat_y_max.x[om, x, z]
                        * np.conjugate(h_scat_y_max.z[om, x, z])
                    )
                )
        ...
        for x in range(scat.x_min, scat.x_max + 1):
            for y in range(scat.y_min, scat.y_max + 1):
                S_scat_DFT[4, om] -= 0.5 * (
                    np.real(
                        e_scat_z_min.x[om, x, y]
                        * np.conjugate(h_scat_z_min.y[om, x, y])
                    )
                    - np.real(
                        e_scat_z_min.y[om, x, y]
                        * np.conjugate(h_scat_z_min.x[om, x, y])
                    )
                )
                S_scat_DFT[5, om] += 0.5 * (
                    np.real(
                        e_scat_z_max.x[om, x, y]
                        * np.conjugate(h_scat_z_max.y[om, x, y])
                    )
                    - np.real(
                        e_scat_z_max.y[om, x, y]
                        * np.conjugate(h_scat_z_max.x[om, x, y])
                    )
                )
    ...
    for om in range(iwdim + 1):
        ...
        for y in range(abs.y_min, abs.y_max + 1):
            for z in range(abs.z_min, abs.z_max + 1):
                S_abs_DFT[0, om] += 0.5 * (
                    np.real(
                        e_abs_x_min.y[om, y, z] * np.conjugate(h_abs_x_min.z[om, y, z])
                        - e_abs_x_min.z[om, y, z]
                        * np.conjugate(h_abs_x_min.y[om, y, z])
                    )
                )
                S_abs_DFT[1, om] -= 0.5 * (
                    np.real(
                        e_abs_x_max.y[om, y, z] * np.conjugate(h_abs_x_max.z[om, y, z])
                        - e_abs_x_max.z[om, y, z]
                        * np.conjugate(h_abs_x_max.y[om, y, z])
                    )
                )
        ...
        for x in range(abs.x_min, abs.x_max + 1):
            for z in range(abs.z_min, abs.z_max + 1):
                S_abs_DFT[2, om] += 0.5 * (
                    np.real(
                        e_abs_y_min.z[om, x, z] * np.conjugate(h_abs_y_min.x[om, x, z])
                        - e_abs_y_min.x[om, x, z]
                        * np.conjugate(h_abs_y_min.z[om, x, z])
                    )
                )
                S_abs_DFT[3, om] -= 0.5 * (
                    np.real(
                        e_abs_y_max.z[om, x, z] * np.conjugate(h_abs_y_max.x[om, x, z])
                        - e_abs_y_max.x[om, x, z]
                        * np.conjugate(h_abs_y_max.z[om, x, z])
                    )
                )
        ...
        for x in range(abs.x_min, abs.x_max + 1):
            for y in range(abs.y_min, abs.y_max + 1):
                S_abs_DFT[4, om] += 0.5 * (
                    np.real(
                        e_abs_z_min.x[om, x, y] * np.conjugate(h_abs_z_min.y[om, x, y])
                        - e_abs_z_min.y[om, x, y]
                        * np.conjugate(h_abs_z_min.x[om, x, y])
                    )
                )
                S_abs_DFT[5, om] -= 0.5 * (
                    np.real(
                        e_abs_z_max.x[om, x, y] * np.conjugate(h_abs_z_max.y[om, x, y])
                        - e_abs_z_max.y[om, x, y]
                        * np.conjugate(h_abs_z_max.x[om, x, y])
                    )
                )
